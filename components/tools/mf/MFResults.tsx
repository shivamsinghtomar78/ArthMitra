...
import { TaxRebalancingExecutor } from "./TaxRebalancingExecutor"
...
export const MFResults = React.memo(function MFResults({ result }: { result?: MFXrayResult }) {
  const [activeTab, setActiveTab] = useState<"overview" | "tax-executor">("overview")
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex border-b border-line pb-px gap-6">
        <button
          onClick={() => setActiveTab("overview")}
          className={`pb-4 text-sm font-bold tracking-wide uppercase transition-all duration-300 relative flex items-center gap-2 ${
            activeTab === "overview" ? "text-brand border-b-2 border-brand" : "text-mutedText hover:text-foreground"
          }`}
        >
        ...
        <button
          onClick={() => setActiveTab("tax-executor")}
          className={`pb-4 text-sm font-bold tracking-wide uppercase transition-all duration-300 relative flex items-center gap-2 ${
            activeTab === "tax-executor" ? "text-brand border-b-2 border-brand" : "text-mutedText hover:text-foreground"
          }`}
        >
          <Zap className="size-4" />
          Tax & Rebalancing Executor
        </button>
      </div>
      ...
      <AnimatePresence mode="wait">
        {activeTab === "overview" ? (
          ...
          <motion.div
            ...
            transition={{ duration: 0.2 }}
          >
            <TaxRebalancingExecutor
              funds={result.parsedFunds}
              recommendedAllocation={result.allocation}
            />
          </motion.div>
        )}
      </AnimatePresence>
